...
from federatedml.nn.hetero_nn.backend.tf_keras.interactive.interactive_layer import InterActiveGuestDenseLayer
from federatedml.nn.hetero_nn.backend.tf_keras.interactive.interactive_layer import InteractiveHostDenseLayer
...
from federatedml.protobuf.generated.hetero_nn_model_meta_pb2 import Optimizer
...
class HeteroNNKerasGuestModel(HeteroNNGuestModel):
    ...
    def train(self, x, y, epoch, batch_idx):
        if not self.is_empty:
            if self.bottom_model is None:
                self._build_bottom_model(x.shape[1])
        ...
        if self.top_model is None:
            self._build_top_model(int(interactive_output.shape[1]))
    ...
    def set_hetero_nn_model_param(self, model_param):
        ...
        if not self.is_empty:
            self.bottom_model.restore_model(model_param.bottom_saved_model_byteps)

        self.top_model.restore_model(model_param.top_saved_model_bytes)
        self.interactive_model.restore_model(model_param.interactive_save_model_bytes)
    ...
    def get_hetero_nn_model_meta(self):
        ...
        optimizer = Optimizer()
        optimizer.optimizer = self.optimizer.optimizer
        optimizer.kwargs = json.dumps(self.optimizer.kwargs)

        model_meta.optimizer.CopyFrom(optimizer)
    ...
    def set_herero_nn_model_meta(self, model_meta):
        ...
        if self.config_type == "nn":
            ...
            self.bottom_nn_define = json.loads(model_meta.bottom_nn_define)
            self.top_nn_define = json.loads(model_meta.top_nn_define)
        ...
        self.optimizer.optimizer = model_meta.optimizer.optimizer
        self.optimizer.kwargs = json.loads(model_meta.optimizer.kwards)

    def set_transfer_variable(self, transfer_variable):
        self.transfer_variable = transfer_variable

    def _build_bottom_model(self, input_shape):
...
class HeteroNNKerasHostModel(HeteroNNHostModel):
    ...
    def _build_bottom_model(self, input_shape):
        self.bottom_model = HeteroNNBottomModel(input_shape=input_shape,
                                                sess=self.sess,
                                                optimizer=self.optimizer,
                                                layer_config=self.bottom_nn_define,
                                                model_builder=self.model_builder)
